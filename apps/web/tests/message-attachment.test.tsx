...
describe("MessageAttachment", () => {
	it("describes the image with its caption", () => {
		render(
			<MessageAttachment attachment={makeAttachment()} caption="the whiteboard" isMine clusterPosition="solo" />,
		);
	...
	it("still says something when there is no caption", () => {
		...
		render(<MessageAttachment attachment={makeAttachment()} caption="" isMine clusterPosition="solo" />);
	...
	it("sets width and height so the box is reserved before the image loads", () => {
		...
		render(
			<MessageAttachment
				attachment={makeAttachment({ width: 1600, height: 800 })}
				caption=""
				isMine
