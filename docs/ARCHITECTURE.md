...
- `MessageReaction` — one person marking one message, once per kind. The composite primary key `(messageId, userId, kind)` **is** the toggle: reacting again is a delete, and the database refuses the duplicate rather than the service checking for one first. `kind` is a Postgres enum of five rather than free text, because a free column makes "the same reaction" undecidable — U+2764 and U+2764 U+FE0F are two strings and one heart. Unique among the things pointing at a `User` here, it is `ON DELETE CASCADE`: a reaction carries no history worth keeping without the person who left it, nothing points at it, and the count simply drops by one. The DTO names everyone (`userIds`) rather than counting, because the message is broadcast to the whole room as one payload — anything answering "is this mine?" would be answering it for whoever triggered the write
...
- `Attachment` — an image on a `Message`, **several per message** since phase 22. `position` stores the order the sender picked, because the rows are written in one transaction and share a `createdAt` to the millisecond — ordering by time would let a gallery shuffle itself between two reads. The unique moved from `messageId` alone to `(messageId, position)` rather than being dropped: two images claiming slot 0 is an ordering the database should refuse, not one the reader discovers. No mime type or original filename column: every upload is re-encoded to WebP, so the type is known and constant, and the name a stranger chose is a string nothing here needs
